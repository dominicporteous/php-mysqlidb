...
class mysqlidb
{
    ...
    public function __construct($host, $username, $password, $db)
    {
        $this->_mysqli = new mysqli($host, $username, $password, $db)
            or die('Connect Error ('.$this->_mysqli->connect_errno.') '.$this->_mysqli->connect_error);
    ...
    {
	    $target = $this->_target;
        $this->_query = "SELECT $target FROM $tableName";
        $stmt = $this->_buildQuery($numRows);
        $stmt->execute();
        $this->reset();
     ...
     * @return mysqlidb
     */
    public function where($whereProp, $whereValue, $whereType = "=")
    {
        $this->_where[$whereProp." ".$whereType] = $whereValue;
        return $this;
    }
	
	/**
     * This method allows you to sort by order of.
     *
     * @uses $mysqlidb->where('id', 7)->where('title', 'MyTitle', '!=')->sorted('title','ASC');
     *
     * @param string $byProp  The name of the database field.
     * @param mixed  $byValue The type of sort. More info:              
     *      http://dev.mysql.com/doc/refman/5.0/en/order-by-optimization.html
     *
     * @return mysqlidb
     */   
    public function sorted($byProp, $byValue=''){
	    $this->_sortParam = array($byProp, $byValue);
	    return $this;
    }
    
    /**
     * This method allows you to return only specific columns.
     *
     * @uses $mysqlidb->where('id', 7)->where('title', 'MyTitle', '!=')->target('title');
     * @uses $mysqlidb->where('id', 7)->where('title', 'MyTitle', '!=')->target('id, time, price');
     *
     * @param string $byProp  The name of the database field.
     * @param mixed  $byValue The type of sort. More info:              
     *      http://dev.mysql.com/doc/refman/5.0/en/order-by-optimization.html
    ...
    /**
     * Method that will compile the WHERE statement,
    ...
    protected function _buildQuery($numRows = null, $tableData = null)
    {
		...
		// Sort Set
        if (count($this->_sortParam)==2) {
            $this->_query .= ' ORDER BY `' . $this->_sortParam[0] . '` ' . $this->_sortParam[1];
        }
